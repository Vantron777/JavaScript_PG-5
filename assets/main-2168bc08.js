import{i as O,a as A}from"./vendor-8cce9181.js";(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const l of i.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&s(l)}).observe(document,{childList:!0,subtree:!0});function t(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function s(n){if(n.ep)return;n.ep=!0;const i=t(n);fetch(n.href,i)}})();const v={openBurgerBtn:document.querySelector(".burger-menu"),closeBurgerBtn:document.querySelector(".mobile-menu__button"),burger:document.querySelector(".mobile-menu-container")};v.openBurgerBtn.addEventListener("click",M);v.closeBurgerBtn.addEventListener("click",M);function M(){v.burger.classList.toggle("is-hidden")}function p(e){document.documentElement.setAttribute("data-theme",e),localStorage.setItem("theme",e),document.querySelectorAll('.common-toggle input[type="checkbox"]').forEach(function(t){e==="dark"?t.checked=!0:t.checked=!1})}const I=localStorage.getItem("theme");p(I||"light");const Z=document.querySelectorAll('.common-toggle input[type="checkbox"]');Z.forEach(function(e){e.addEventListener("change",function(o){o.target.checked?p("dark"):p("light")},!1)});const q=document.querySelector('.toggle-theme input[type="checkbox'),k=localStorage.getItem("theme");k&&(document.documentElement.setAttribute("data-theme",k),k==="dark"&&(q.checked=!0));q.addEventListener("change",P,!1);function P(e){e.target.checked?(document.documentElement.setAttribute("data-theme","dark"),localStorage.setItem("theme","dark")):(document.documentElement.setAttribute("data-theme","light"),localStorage.setItem("theme","light"))}function N(e){return e.map(t=>{const{_id:s,book_image:n,title:i,author:l}=t;return`<li class="books-box-itm" id="${s}">
      <div class="books-box-wrap">
            ...
            </li>`}).join("")}const $=(e,o)=>{if(!e||e.length===0){V("Sorry, books not found");return}else return`<h2 class="books-box-title">Best Sellers <span class="books-title-span"> Books</span></h2>${e.map(s=>{const n=s.list_name,i=N(s.books.slice(0,o));return`<div class="books-box-holder">
      ...
      </div>`}).join("")}`},_=e=>` <li class="categories-itm js-categories-current ">
  <a href="#" data-categoryName="all categories">All categories</a></li>${e.sort((t,s)=>t.list_name<s.list_name?-1:t.list_name>s.list_name?1:0).map(t=>`<li class="categories-itm"><a href="#" data-categoryName="${t.list_name}">${t.list_name}</a></li>`).join("")}`,ee=(e,o)=>{if(!e||e.length===0){V("Sorry, category of book not found");return}else{const t=N(e),s=o.split(" "),n=s[s.length-1];return`<h2 class="books-box-title">
   ${s.slice(0,-1).join(" ")} 
   <span class="books-title-span">${n}</span>
 </h2>
 <div class="books-category-box">
   <ul class="books-category-list">${t}</ul>
 </div>`}};function V(e){O.error({message:e,messageColor:"white",backgroundColor:"red",position:"topRight"})}const m="https://books-backend.p.goit.global",te=async e=>{const t=m+"/books/top-books/";try{const s=await A.get(t);return $(s.data,e)}catch(s){return console.error("Error fetching top books:",s),""}},oe=async()=>{const o=m+"/books/category-list/";try{const t=await A.get(o);return _(t.data)}catch(t){console.error("Error fetching category list:",t)}},se=async e=>{const t=m+"/books/category/",s={category:e};try{const n=await A.get(t,{params:s});return ee(n.data,e)}catch(n){console.error("Error fetching books by category:",n)}},H=async e=>{const o=`/books/${e}`,t=m+o;try{return(await A.get(t)).data}catch(s){console.error("Error fetching books id:",s)}},z="/JavaScript_PG-5/assets/sprite-eb1087d7.svg",ne="addBook";function ie(e){const o=JSON.parse(localStorage.getItem(ne)),t=o&&o.some(s=>s._id===e._id);return`
      ...
      <div class="modal-content">
        ...
        <svg class="close-btn-icon" width="16" height="16">
          <use href="${z}#icon-x-close-black"></use>
    ...
    <div class="modal-buttons">
      ...
      <p class="modal-message ${t?"":"hidden"}">Congratulations! You have added the book to the shopping list. To delete, press the button “Remove from the shopping list”.</p>
  ...
  `}function re(e){var t;const o={amazon:"Amazon",appleBooks:"Apple Books"};for(const[s,n]of Object.entries(o))o[s]=(t=e.find(i=>i.name===n))==null?void 0:t.url;return`
          ...
          /><div class="shoppinglist-cart-content">
            ...
            <button type="button" class="shoppinglist-book-deletebutton">
              <svg class="shoppinglist-btn-svg" width="18" height="18">
                <use href="${z}#icon-trash_bin"></use>
              </svg>
            </button>
          </div>
        </li>`).join("");D.insertAdjacentHTML("afterend",'<div class="shopinglist-cart"><ul class="shoppinglist-list"></ul></div>'),document.querySelector(".shoppinglist-list").innerHTML=o}function K(){document.querySelector(".shoppinglist-list")&&document.querySelector(".shoppinglist-list").remove(),j();const e=`<p class="shopping-list-text">
    ...
    <img
      ...
      src="${Ee}"
      alt="a lot of books"
    />`;D.insertAdjacentHTML("afterend",e),O.info({message:"The list of books is empty.",timeout:3e3,position:"topRight"})}function j(){document.querySelector(".shopping-list-img")&&(document.querySelector(".shopping-list-img").remove(),document.querySelector(".shopping-list-text").remove())}document.querySelector(".shopping-list").addEventListener("click",e=>{if(e.target.nodeName=="BUTTON"){const o=e.target.closest("li").getAttribute("id");console.log(o);const t=a().filter(n=>n._id!==o);addtoLS(t),document.querySelector(`li[id="${o}"]`).remove()}(!a()||a().length===0)&&K()});a()&&a().length>0?Se(a()):K();window.addEventListener("load",()=>{setTimeout(()=>{},350)});document.querySelector(".loader");function Be(){window.scrollTo({top:0,behavior:"smooth"})}const u=document.querySelector(".top-btn");u.style.display="none";window.addEventListener("scroll",()=>{window.scrollY>window.innerHeight?u.style.display="inline-flex":u.style.display="none"});u.addEventListener("click",Be);
//# sourceMappingURL=main-2168bc08.js.map
